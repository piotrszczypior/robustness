...
# import matplotlib
import logging
from loader import ImageNetDataModule, DatasetConfig, DatasetType
...
if __name__ == "__main__":
    ...
    # test()
    resnet152 = models.get_resnet152()
    resnet152.to(DEVICE)
    imagenet = ImageNetDataModule.get_dataset(DatasetConfig(type=DatasetType.IMAGENET))
    loader = DataLoader(
        dataset=imagenet, batch_size=Config.BATCH_SIZE, num_workers=Config.NUM_WORKERS
    )

    print("Starting evaluation...")
    acc_per_class = evaluate_per_class(model=resnet152, loader=loader)

    os.makedirs("results", exist_ok=True)

    # Create a DataFrame for better visualization and saving
    df = pd.DataFrame(
        {"class_id": range(len(acc_per_class)), "accuracy": acc_per_class}
    )

    output_path = "results/per_class_accuracy_resnet152.csv"
    df.to_csv(output_path, index=False)
    print(f"Evaluation complete. Results saved to {output_path}")
    print(f"Mean accuracy: {acc_per_class.mean():.4f}")
