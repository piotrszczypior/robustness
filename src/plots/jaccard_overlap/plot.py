...
class DomainJaccardOverlapPlot(BasePlotPipeline):
    ...
    def _get_classes_set(self, corrupted_acc: pd.DataFrame, top_k: int, tail: str) -> set:
        if tail == "best":
            sorted_acc = corrupted_acc.sort_values(by=["accuracy", "y_true"], ascending=[False, True])
        else:
            sorted_acc = corrupted_acc.sort_values(by=["accuracy", "y_true"], ascending=[True, True])
