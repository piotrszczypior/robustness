...
import argparse
import logging
from .settings import get_spearman_plot_specs, get_averaged_spearman_plot_specs
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument("--mode", default="default", choices=["default", "average"], help="Plotting mode: 'default' for individual corruptions, 'average' for grouped corruptions.")
...
def run(args: argparse.Namespace):
    if args.mode == "average":
        logger.info(
            f"Generating averaged spearman plots for groups: {args.corruptions}"
        )
        plots = get_averaged_spearman_plot_specs(
            metric_type=args.metric, groups=args.corruptions, severities=args.severities
        )
        for cng in plots:
